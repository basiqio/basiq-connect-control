...
const authRequestIdValidationFailed = ({ value }) => ({
...
const smsCodeValidationFailed = ({ value }) => ({
	type: actionTypes.SMS_CODE_VALIDATION_FAILED,
	value
});
const smsCodeSent = ({ value }) => ({
...
const tokenValidationFailed = ({ value }) => ({
...
export const connectToBank =
	(selectedInstitution, institutionId, loginId, password, securityCode, secondaryLoginId) => async (dispatch, getState) => {
		...
		const abortController = apiService.addJobStatusChangedCallback(response.payload.id, getState, (status) => {
			...
			if (verifyCredentialsStep) {
				if (verifyCredentialsStep.status === 'failed') {
...
export const fetchInstitutions = (hideBetaBanks, institutionRegion, tokenVersion) => async (dispatch) => {
	const institutions = await apiService.getInstitutions(hideBetaBanks, institutionRegion, tokenVersion);
...
export const validateAuthRequestId =
	({ connectLink, connect, upload, partnerName, institutionRegion, hideTestBanks, hideBetaBanks }) =>
	async (dispatch) => {
		if (!connectLink) {
			dispatch(
				authRequestIdValidationFailed({
					value: 'Invalid link'
				})
			);
		} else {
...
export const validateToken =
	({ token, userId, connect, upload, partnerName, institutionRegion, hideTestBanks, hideBetaBanks }) =>
	async (dispatch) => {
...
export const showBankConnect =
	({ userId, accessToken, connectSupported, uploadSupported }) =>
	(dispatch) => {
...
export const verifySmsCode = (authRequestId, smsCode, hideBetaBanks, institutionRegionValue) => async (dispatch) => {
