...
const bankConnectFailed = (value) => ({ type: actionTypes.BANK_CONNECT_FAILED, value });
...
export const validateAuthRequestId = ({ connectLink, connect, upload, partnerName, institutionRegion, hideTestBanks, hideBetaBanks }) => async (
	dispatch
) => {
	if (!connectLink) {
...
export const validateToken = ({ token, userId, connect, upload, partnerName, institutionRegion, hideTestBanks, hideBetaBanks }) => async (
	dispatch
) => {
