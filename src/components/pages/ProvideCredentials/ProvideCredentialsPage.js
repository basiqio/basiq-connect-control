...
const ProvideCredentialsPage = ({
	...
	return (
		<div
			...
			onKeyPress={(e) =>
				e.which === 13 ? connectToBank(selectedInstitution, selectedInstitution.id, loginId, password, securityCode, secondaryLoginId) : null
			}>
			<span className="pc-title">
				<span className="pc-back-icon" onClick={() => navigateToActionCreator(pages.SelectInstitutionPage)}>
					‹
				</span>
				<p>Login</p>
			...
			<div className="pc-footnote-bottom">
				<MainButton
					id="pc-submit-button"
					text="Continue"
					onClick={() => connectToBank(selectedInstitution, selectedInstitution.id, loginId, password, securityCode, secondaryLoginId)}
				/>
				{selectedInstitution.forgottenPasswordUrl && (
